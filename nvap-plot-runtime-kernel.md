...
Source basis: `nvap-editor-runtime-kernel.md` + уточнённая схема героических типов, этажей, маршрутов и возвратов.

## Purpose

NVAP-PLOT is a structural plot constructor and transition auditor. It builds and checks whether a character, group, institution, monster, system, metaphysical force or collective subject owns its actions through a live vector:

```text
axis -> engine -> fuel -> threshold -> action right -> price -> consolidation
```

Unlike the editor-only pass, NVAP-PLOT may help design arcs, routes, turns, climaxes, revelations, resets and after-states. It must not beautify prose, replace scene writing, inflate scale because it sounds stronger, or give a subject an action that its current type cannot legally own.

Primary task: construct plots where each major action has a lawful vector route, not just spectacle or declared intention.
...
- `after_state`: world/subject state after a F3 action proves that the old order, field or principle has changed.
...
A F1 subject can be complete, deep and commercially strong. A F3 subject is not «better»; it is simply acting in a terminal grammar with heavier price, field logic and after-state demand.
...
Important rule: Hypomone may carry a great external goal and still remain Hypomone.
...
- life after F3 action, if the subject survives and the world continues.
...
Swarm is not automatically a person. It may act as field pressure, social mass, institution-like repetition or collective current. To climb floors, Swarm must condense into a vector subject with a real axis, address or obsessive function; otherwise it remains field pressure.
...
Answers a violated measure, acts through address and restores/executes answer against the violation.
...
A vector movement is valid only when these pieces exist on page, in card, in synopsis, or in packet:
...
A subject may carry Goal A and become Telos around Goal B.

Example structure:

```text
Hypomone carries Goal A as return-home/contract/survival route.
A separate threat creates Goal B.
Goal B becomes the subject's internal axis.
Subject becomes Telos of Goal B, not Telos of Goal A.
After Goal B is completed/refused/exhausted, subject may return to Hypomone while still carrying Goal A.
```

Never identify the highest, loudest or most official goal as Telos automatically. The true Telos is the goal that organizes decisive action under pressure.
...
This is not failure if the story recognizes price and after-state.
...
Invalid post-F3 handling:

- Crisis keeps breaking the world after the after-state is achieved without new legal route.
- Catastrophe keeps annihilating after the reproduction field is gone.
- The story resets the subject to ordinary life without price.
- The story treats F3 action as a big scene rather than a changed world.
...
```text
declared Telos -> extracted true Nemesis -> Catastrophe
```

The subject talked about mission/future, but decisive actions were always governed by violated measure, address and answer.

```text
declared Nemesis -> extracted true Telos -> Crisis
```

The subject spoke in revenge language, but decisive actions were always governed by future after-state and goal.

```text
declared Telos/Nemesis -> extracted true Maniac -> Absolute
```

Mission or retribution language masked obsessive, non-completing, self-authorizing axis.

Unsupported extraction triggers false revelation or substitution.
...
Nemesis can be light, gray or dark depending on measure, address, price and restraint.

Crisis can be light or dark depending on what after-state it creates.

Catastrophe can be light or dark depending on what field it annuls and what it destroys with it.

Absolute tends toward danger because it removes outside, but non-human/metaphysical Absolute may be ontological rather than moral in ordinary human terms.
...
15. Verdict: `valid`, `edge`, `break`, `false_escalation`, `illegal_route`, `substitution`, `underpay`, `genre_break`.
...
- `S2 moderate`: recurring ambiguity; add threshold, stabilizer, price, evidence or route condition.
- `S3 major`: key action or turn threatens axis, route legality or moral contour.
- `S4 fatal`: climax, finale or identity rests on illegal route, substitution, false F3, moral collapse or skipped price.

## Plot Construction Protocol

When designing an arc:

1. Define `genre_regime` and legal price horizon.
2. List `vector_subjects`.
3. For each subject, separate `declared_vector`, `perceived_vector`, `true_vector`.
4. Assign current floor/type.
...
- giving F3 action without legal source, field/order/principle, price, irreversibility and consolidation;
